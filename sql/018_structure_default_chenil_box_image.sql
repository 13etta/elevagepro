-- Image par défaut pour les chenils / box.
-- L'image est servie depuis les assets publics de l'application.
...
SET image_url = '/images/structure/default-chenil-box.svg',
    updated_at = CURRENT_TIMESTAMP
WHERE lower(COALESCE(type, '')) IN ('box', 'chenil', 'kennel')
   OR lower(COALESCE(name, '')) LIKE '%box%'
   OR lower(COALESCE(name, '')) LIKE '%chenil%'
   OR lower(COALESCE(name, '')) LIKE '%kennel%';
...
BEGIN
  ...
  IF (
    lower(COALESCE(NEW.type, '')) IN ('box', 'chenil', 'kennel')
    OR lower(COALESCE(NEW.name, '')) LIKE '%box%'
    OR lower(COALESCE(NEW.name, '')) LIKE '%chenil%'
    OR lower(COALESCE(NEW.name, '')) LIKE '%kennel%'
  )
  AND (
    NEW.image_url IS NULL
    OR trim(NEW.image_url) = ''
    OR NEW.image_url = 'https://images.unsplash.com/photo-1601758228041-f3b2795255f1?auto=format&fit=crop&w=1600&q=82'
  ) THEN
    NEW.image_url := '/images/structure/default-chenil-box.svg';
  END IF;
