...
// Requête récursive pour obtenir la généalogie d'un chien
...
exports.runSimulation = async (req, res) => {
    try {
        ...
        // 1. Récupération des noms des parents
        ...
        // 2. Extraction des arbres généalogiques (5 générations)
        const sirePedigree = await getPedigree(sire_id, 5);
        const damPedigree = await getPedigree(dam_id, 5);

        // 3. Algorithme de recherche des ancêtres communs et calcul de Wright
        let coi = 0;
        ...
        sirePedigree.forEach(sireAncestor => {
            damPedigree.forEach(damAncestor => {
                if (sireAncestor.id === damAncestor.id) {
                    // Formule de Wright simplifiée : (1/2)^(n1 + n2 + 1)
        ...
        // 4. Recharger les listes pour le formulaire
